		...
		*/
		...
		for(var i=0; i<6;i++){
			if(scroll>=ht*i && scroll< ht*(i+1)){
				...
				$("#gnb li").removeClass();
				$("#gnb li").eq(i).addClass("current");
			};
		}
	});


	// $("section").first().on("mousewheel",function(event,delta){
	// 	if(delta>0 && $(this).first()){
	// 		var last = $(this).last().offset().top;
	// 		$("html,body").stop().animate({"scrollTop":last},1400);
	// 	}
	// })
	
    //section위에서 마우스 휠을 움직이면
    $("section").on("mousewheel",function(event,delta){    
        //마우스 휠을 위로 올렸을 때 -> delta=1
        //마우스 휠을 아래로 내렸을 때 -> delta=-1
			if(delta>0 && $("section:eq(0)")){
				var last = $("section:eq(5)").offset().top;
				$("html,body").stop().animate({"scrollTop":last},1400)
			}else if(delta<0 && $("section:eq(5)")){
				var first = $("section:eq(0)").offset().top;
				$("html,body").stop().animate({"scrollTop":first},1400)
			}
        //마우스 휠을 올렸을때	
